    ...
    loadPreviousPayments = async() => {
        ...
        try {
            let response = await axios.get(
                `${config.apiPath}/GetPreviousReimbursements?employeeId=${
                    EmployeeId
                }&collegeCode=${
                    CollegeCode
                }&reimbursementYear=${
                            ...
                            <table>
                                <thead>
                                    <tr>                                        
                                        ...
                                        <th className='text-right'>Difference from <br></br> Previous</th>
                                        <th className='text-right'>Salary Reimbursed</th>
                                ...
                                <tbody>                               
                                    {listPreviousPayments.map((item, index) => (                                    
                                            <tr key={index}>                                                
                                                ...
                                                <td>{utils.formatDate(item.ImportDate)}</td>
                                                <td>{utils.formatDate(item.PayPeriodEnding)}</td>
                                                <td>{utils.formatDate(item.LastPayDate)}</td>
                                                <td className='text-right'>{utils.currency(item.YTDPaid)}</td>
                                                <td className='text-right'>{utils.currency(item.YTDDiff)}</td>
                                                <td className='text-right'>{utils.currency(item.SalaryReimbursed)}</td>
                                                <td>{utils.formatDate(item.GLPostingDate)}</td>
                                            </tr>                                                                 
                                    ))}                                
                                </tbody>                           
                            </table>
